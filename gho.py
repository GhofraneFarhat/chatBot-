...
with jsonlines.open("professeur.jsonl") as f:
    data = list(f)
# Diviser les données en entrées (X) et sorties (y)
questions = [d['Nom et Prénom'] for d in data]
reponses = [d['Info'] for d in data]
...
while True:
    ...
    speak_answer("votre question")
    speak_answer(result["question"])
    speak_answer("Question Proposé est")
